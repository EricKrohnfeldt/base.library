...
/**
 * Super class test module for a failing {@link Mockish} implementations.
 * @param <M> The {@link Mockish} implementation to test
 */
public abstract non-sealed class MockishFailingTest<M extends Mockish>
	...
	protected abstract Runnable prepareMock( M mock, RuntimeException exception );
	...
	class autoValidate {
		...
		private DynamicTest failing( String name, Supplier<M> constructor, Consumer<M> validate ) {
			return DynamicTest.dynamicTest( name, () -> {
				// Arrange
				M mock = constructor.get();
				prepareMock( mock, randomException() );
				// Act
				try {
					validate.accept( mock );
	...
	private void prepareAndExecute( M mock ) {
		RuntimeException expected = randomException();
		executeCall(
			prepareMock( mock, expected ),
			expected
		);
	}

	private void executeCall( Runnable call, RuntimeException expected ) {
		try {
			call.run();
			Assertions.fail();
		}
	...
	final RuntimeException randomException() {
