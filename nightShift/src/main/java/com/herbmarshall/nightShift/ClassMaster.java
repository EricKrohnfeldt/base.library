...
final class ClassMaster<C> {

	private final Class<C> real;

	private ClassMaster( Class<C> real ) {
		this.real = Objects.requireNonNull( real );
	}

	boolean isJob() {
		return AutomatedJob.class.isAssignableFrom( real );
	}

	static <C> ClassMaster<C> scrutinize( Class<C> real ) {
		return new ClassMaster<>( real );
	}
